...
- 2026-04-05: Implemented "Open Repository" flow. Created WelcomeScreen allowing users to pick repo or select from recent repos dropdown. Handled drag-and-drop loading repo. Extended the Rust backend with `open_repo`, `get_repo_status`, and recent repository caching logic (read/write JSON to app_data_dir). Updated `store.ts` to sync complete repo information cleanly via `loadRepo` method. 
